...
# loading data
known <- load_rda(file.path(data.folder, "known_structured_basicf.rda"))
data_unknown <- load_rda(file.path(data.folder, "unknown_structured_basicf.rda"))
...
# merge 
data <- as.data.table(rbind(known[, c("ID", "Month", "SalOrg", "Material", "date", "t", "demand", 
                        colnames(known)[grepl("demand_", colnames(known))])], unknown))
...
cols <- c("Month", colnames(known)[grepl("demand", colnames(known))], colnames(known)[grepl("demand_", colnames(known))])
known <- as.data.frame(known)
known <- known[, colnames(known) %in% cols]
known3 <- na.omit(known[, c("Month", colnames(known)[grepl("demand", colnames(known))])])
known3 <- na.omit(known3[, !(colnames(known3) %in% paste0(".demand_lag_", 1:2))])

# unknown data with lags only
cols <- c("ID", colnames(unknown)[grepl(".demand", colnames(unknown))], colnames(unknown)[grepl("demand", colnames(unknown))])
unknown <- as.data.frame(unknown)
unknown <- unknown[, colnames(unknown) %in% cols]
unknown3 <- unknown[, colnames(unknown)[grepl("demand_", colnames(unknown))]]
unknown3 <- unknown[, !(colnames(unknown3) %in% c(paste0(".demand_lag_", 1:2)))]
...
write.table(as.data.frame(subm), file.path(subm.folder, "xgb_lags_1model.csv"), sep = ",", row.names = F, quote = F)